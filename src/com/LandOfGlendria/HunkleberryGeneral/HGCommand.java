...
public enum HGCommand {
	...
	SIGHTS					("sights",			"hg.loc",				true,	"",	true,	"{set|remove|list}[name:req. for set|remove]",																		"Get your current x y z location."),
	SEE						("see",				"hg.loc",				true,	"",	true,	"{name}",																		"Get your current x y z location."),
	...
	WORLD_LOAD_LIST			("worldloadlist",	"hg.worlds",			true,	"",	true,	"{add|remove|list|save}",												"Adds, removes, lists or saves entries in worldly.properties to affect world loading on startup(plugin enable), changes must be saved before. Will only add a world that you currently occupy."),
